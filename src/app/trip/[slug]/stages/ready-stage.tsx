...
export function ReadyStage({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-center py-8 relative">
        ...
        <div className="confetti-container">
          {Array.from({ length: 30 }).map((_, i) => {
            const colors = ["#E8622B", "#F5A623", "#1D7A54", "#7C3AED", "#0D9488", "#DC2626"];
            const color = colors[i % colors.length];
            const left = 20 + Math.random() * 60; // 20-80% horizontal spread from center
        ...
        <div className="text-5xl mb-4 animate-celebrate" style={{ animationDelay: "0.1s" }}>🎉</div>
        <h2 className="font-heading text-2xl font-extrabold text-text animate-celebrate" style={{ animationDelay: "0.25s" }}>
          {currentMember?.name ? `${currentMember.name}, it's happening!` : "Trip is a go!"}
        </h2>
        <p className="text-text-secondary text-sm mt-2 animate-in" style={{ animationDelay: "0.5s" }}>
      ...
      <button
        onClick={handleCopy}
        className="w-full bg-whatsapp text-white rounded-2xl px-5 py-4 text-sm font-bold shadow-sm hover:bg-whatsapp-hover active:scale-[0.98] transition-all flex items-center justify-center gap-2.5 animate-delayed-cta"
      >
